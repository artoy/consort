  ...
  public TranslatedBasicBlock(BasicBlock basicBlock, boolean headOfFunction) {
    ...
    for (int i = 0; i < basicBlock.units.size(); i++) {
      ...
      if (i == basicBlock.units.size() - 1){
        ...
        TranslatedUnit tailTranslatedUnit = new TranslatedUnit(basicBlock.units.get(i), headOfFunction, indentLevel);
        translatedBasicBlock.add(tailTranslatedUnit);
      } else {
        TranslatedUnit translatedUnit = new TranslatedUnit(basicBlock.units.get(i), headOfFunction, indentLevel);

        // 逐次実行かそうでないかでインデントと {} を付ける
        if (prevSequence == false && translatedUnit.getIsSequencing() == true) {
          translatedUnit.beginSequencing = true;
          translatedUnit.endSequencing = false;
          translatedUnit.indentLevel++;
        } else if (prevSequence == true && translatedUnit.getIsSequencing() == false) {
